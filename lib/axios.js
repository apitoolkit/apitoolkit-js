...
const onResponse = (urlWildcard, redactHeaderLists, redactRequestBody, redactResponseBody, notWebContext) => (response) => {
    try {
        ...
        const parent_id = apitoolkit_1.asyncLocalStorage.getStore().get("AT_msg_id");
        const errors = [];
...
const onResponseError = (urlWildcard, redactHeaderLists, redactRequestBody, redactResponseBody, notWebContext) => (error) => {
    try {
        if (apitoolkit_1.asyncLocalStorage.getStore() == null && !notWebContext) {
            console.log("APIToolkit: observeAxios used outside of the APIToolkit middleware's scope. Use the APIToolkitClient.observeAxios instead, if you're not in a web context.");
            return Promise.reject(error);
        ...
        const respBody = typeof res?.data === "string"
            ...
            : JSON.stringify(res?.data || {});
        const project_id = apitoolkit_1.asyncLocalStorage.getStore().get("AT_project_id");
        const ATClient = apitoolkit_1.asyncLocalStorage
            .getStore()
            .get("AT_client");
        const ATConfig = apitoolkit_1.asyncLocalStorage.getStore().get("AT_config");
        const parent_id = apitoolkit_1.asyncLocalStorage.getStore().get("AT_msg_id");
...
function observeAxios(axiosInstance, urlWildcard = undefined, redactHeaders = [], redactRequestBody = [], redactResponseBody = [], notWebContext = false) {
    axiosInstance.interceptors.request.use(exports.onRequest, exports.onRequestError);
    axiosInstance.interceptors.response.use((0, exports.onResponse)(urlWildcard, redactHeaders, redactRequestBody, redactResponseBody, !!notWebContext), (0, exports.onResponseError)(urlWildcard, redactHeaders, redactRequestBody, redactResponseBody, !!notWebContext));
